...
| Games & fun | **Mystery Maker** | Build an ARG-style treasure hunt as a chain: you hand out one START.txt and every stage hides the way to the next (the tool writes the "go to… / password…" wiring itself). Stages: ciphers (Caesar, Vigenère, Base64, Morse, invisible ink, three layers), messages in PNG pixels/metadata, words drawn in a WAV spectrogram, password zips, decoy folder mazes, QR codes; ends on a page with an unlisted video. Export as a folder of files, or as a static website with unguessable page addresses and password pages that decrypt the next link in the browser. Easy/Medium/Hard templates, live walkthrough, solution sheet, decode-anything tab. | Working |
